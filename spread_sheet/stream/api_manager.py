...
from spread_sheet.data_model.sheet import Sheet, SpreadSheets
...
class ApiManager:
    ...
    def __get_sheets(self) -> SpreadSheets:
        response = requests.get(
            "https://open.feishu.cn/open-apis/sheets/v3/spreadsheets/{0}/sheets/query".format(
                self.table_id),
    ...
    def froze_sheet(self, sheet_id: str, column: int = 0, row: int = 0, index: int = 1, ):
        print("froze_sheet {}".format("start froze"))
        response = requests.post(
            "https://open.feishu.cn/open-apis/sheets/v2/spreadsheets/{0}/sheets_batch_update".format(
                self.table_id),
            headers=self.headers,
            data=json.dumps({
                "requests": [
    ...
    def add_rows(self, count: int, sheet_id: str):
        response = requests.post(
            "https://open.feishu.cn/open-apis/sheets/v2/spreadsheets/{0}/dimension_range".format(
                self.table_id),
            headers=self.headers,
            data=json.dumps(
                {
    ...
    def clean_sheet_style(self, sheet_id: str):
        response = requests.put(
            "https://open.feishu.cn/open-apis/sheets/v2/spreadsheets/{0}/styles_batch_update".format(
                self.table_id),
            headers=self.headers,
            data=json.dumps({
                "data": {
    ...
    def write_values(self, values, offset_x: int, offset_y: int, sheet_id: str):
        ...
        response = requests.put(
            "https://open.feishu.cn/open-apis/sheets/v2/spreadsheets/{0}/values".format(
                self.table_id),
            headers=self.headers,
            data=json.dumps(
                {
    ...
    # 写入背景色
    def write_bg_color(self, width: int, length: int, offset_x: int, offset_y: int, sheet_id: str,
                       color=''):
    ...
    # 设置下拉列表
    def set_pull_list(self, values, width: int, height: int, offset_x: int, offset_y: int,
                      sheet_id: str, multi=True, ):
        offset_x = chr(ord('A') + offset_x)
        offset_y += 1
        ...
        response = requests.post(
            "https://open.feishu.cn/open-apis/sheets/v2/spreadsheets/{0}/dataValidation".format(
                self.table_id),
            headers=self.headers,
            data=json.dumps(
                {
    ...
    def merge_column(self, length: int, offset_x: int, offset_y: int, sheet_id: str):
        ...
        response = requests.post(
            "https://open.feishu.cn/open-apis/sheets/v2/spreadsheets/{0}/merge_cells".format(
                self.table_id),
            headers=self.headers,
            data=json.dumps(
                {
    ...
    @classmethod
    def color_style(cls, width: int, length: int, offset_x: int, offset_y: int, sheet_id: str,
                    color='', ):
        offset_x = chr(ord('A') + offset_x)
        offset_y += 1
        ran = "{0}!{1}:{2}".format(
    ...
    def __link_row(cls, index: int):
        ...
        # 硬编码0 - 9999
        levels = {
            0: {'1': 'E', '2': 'I', '3': 'M', '4': 'Q', '5': 'U', '6': 'Y', '7': 'Tc', '8': 'Tg',
                '9': 'Tk'},
            1: {'0': 'w', '1': 'x', '2': 'y', '3': 'z', '4': '0', '5': '1', '6': '2', '7': '3',
                '8': '4', '9': '5'},
            2: {'0': 'MA', '1': 'MQ', '2': 'Mg', '3': 'Mw', '4': 'NA', '5': 'NQ', '6': 'Ng',
                '7': 'Nw', '8': 'OA',
                '9': 'OQ'},
            3: {
                '0': 'A', '1': 'E', '2': 'I', '3': 'M', '4': 'Q', '5': 'U', '6': 'Y', '7': 'c',
                '8': 'g', '9': 'k',
                '00': 'MD', '10': 'MT', '20': 'Mj', '30': 'Mz', '40': 'ND', '50': 'NT', '60': 'Nj',
                '70': 'Nz',
                '80': 'OD', '90': 'OT'
            },
        }
